const bahanModels = require('./bahanModels');
const productModels = require('./productModels');
const bomModels = require('./bomModels');
const orderModels = require('./orderModels');
const vendorModels = require('./vendorModels');
const rfqModels = require('./rfqModels');
const purchaseOrderModels = require('./purchaseOrderModels');
const billModels = require('./billModels');
const customerModels = require('./customerModels');
const quotationModels = require('./quotationModels');
const salesOrderModels = require('./salesOrderModels');
const salesInvoiceModels = require('./salesInvoiceModels');

module.exports = {
    bahanModels,
    productModels,
    bomModels,
    orderModels,
    vendorModels,
    rfqModels,
    purchaseOrderModels,
    billModels,
    customerModels,
    quotationModels,
    salesOrderModels,
    salesInvoiceModels,
}